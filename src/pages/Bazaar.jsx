...
export default function Bazaar() {
    ...
    return (
        <div className="bg-white p-4 sm:p-6 lg:p-10 rounded-2xl sm:rounded-3xl lg:rounded-[32px] shadow-sm border border-slate-50">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-2.5 sm:gap-3 lg:gap-4">
                {daysOfWeek.map((day, idx) => {
                    const assignedMemberId = utilities[`bazaar_${idx}`] || '';
                    const assignedMember = members.find(m => m.id === assignedMemberId);

                    return (
                        <div key={idx} className={`p-3 sm:p-4 lg:p-5 rounded-xl sm:rounded-2xl border ${dayColors[idx]} transition-all`}>
                            ...
                            {isAdmin ? (
                                <select
                                    ...
                                    className="w-full px-2 sm:px-3 lg:px-4 py-2 sm:py-2.5 lg:py-3 rounded-lg sm:rounded-xl border border-slate-200 bg-white font-bold text-slate-800 focus:ring-2 focus:ring-slate-900 text-xs sm:text-sm"
                                >
                                ...
                                <div className="flex items-center gap-2 sm:gap-3">
                                    <div className="w-7 h-7 sm:w-8 sm:h-8 lg:w-10 lg:h-10 rounded-full bg-white flex items-center justify-center border border-slate-200 shrink-0">
